...
class MockProvider:
    ...
    async def complete(self, protocol: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if protocol == "anthropic":
            return {
                ...
                "content": [{"type": "text", "text": "mock response"}], "stop_reason": "end_turn",
                "usage": {"input_tokens": 1, "output_tokens": 2},
            }
        return {
            "id": "chatcmpl_mock", "object": "chat.completion", "created": 0, "model": payload["model"],
    ...
    async def stream(self, protocol: str, payload: dict[str, Any]) -> AsyncIterator[str]:
        ...
        if protocol == "anthropic":
            yield "event: content_block_delta\ndata: " + json.dumps({"type": "content_block_delta", "delta": {"type": "text_delta", "text": "mock response"}}) + "\n\n"
            yield "event: message_stop\ndata: {\"type\": \"message_stop\"}\n\n"
        else:
            chunk = {"id": result["id"], "object": "chat.completion.chunk", "model": result["model"], "choices": [{"index": 0, "delta": {"content": "mock response"}, "finish_reason": "stop"}]}
