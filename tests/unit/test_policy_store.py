...
def test_policy_versions_survive_store_reopen_and_are_immutable(tmp_path: Path) -> None:
...
def test_new_teams_default_to_shadow_and_persist_explicit_setting(tmp_path: Path) -> None:
    database = str(tmp_path / "policies.sqlite3")
    ...
    reopened = PolicyStore(database, default_ruleset())
